...
int main(int argc, char** argv) {
    ...
    // should update so that there is a second argument, which camera is being used 
    ...
    // identify the bad part and remove it
    if (boxInspector2.get_bad_part_Q1(current_part)) {
    ...
    // replace the faulty part that was removed
    ...
    // inspect all parts in the box and make sure they are in the right locations within tolerance
...
////////////////////////////////////////////////////////////////////////////////////////////
    ...
    // move conveyor from Q1 station to Q2 station- NOW WORK SHOULD BE DONE AT Q2
    ...
    //update box pose,  if possible             
    // should update so that there is a second argument, which camera is being used 
    if (boxInspector2.get_box_pose_wrt_world2(box_pose_wrt_world)) {
        ROS_INFO_STREAM("box seen at: " << box_pose_wrt_world << endl);
    }
    else {
        ROS_WARN("no box seen.  something is wrong! I quit!!");
    ...
    // inspect the box at Q2 and classify all the observed parts
    boxInspector2.update_inspection(desired_models_wrt_world,
        satisfied_models_wrt_world,misplaced_models_actual_coords_wrt_world,
        misplaced_models_desired_coords_wrt_world,missing_models_wrt_world,
        orphan_models_wrt_world,part_indices_missing,part_indices_misplaced,
    ...
    // identify the bad part from Q2 and remove it
    if (boxInspector2.get_bad_part_Q2(current_part)) {
        ROS_INFO("found bad part: ");
        ROS_INFO_STREAM(current_part<<endl);
    ...
    nparts = orphan_models_wrt_world.size();
    while (nparts> 0) {
       model_to_part(orphan_models_wrt_world[0], current_part, inventory_msgs::Part::QUALITY_SENSOR_2);
    ...
    // replace the faulty part that was removed at Q2
    n_missing_parts = part_indices_missing.size();
    while (n_missing_parts > 0) {
        int n_missing_part = part_indices_missing[0];
        std::string part_name(desired_models_wrt_world[n_missing_part].type);
    ...
    // advance box from Q2 to drone dock- NOW IS HEADING TO DRONE PICK UP LOCATION
    ...
    // check to see when box has reached the drone dock
    ...
    // call drone based on drone sensor
